...
router.post('/manage/user/delete', (req, res) => {
  ...
  console.log('已删除此ID的用户:', req.body)
...
router.get('/manage/product/search', (req, res) => {
  const {pageNum, pageSize, searchName, productName, productDesc,productCategoryId,productDetail,productPrice,productStatus} = req.query
  let contition = {}
  if (productName) {
    ...
    contition = {categoryId: new RegExp(`^.*${productCategoryId}.*$`)}
  }else if (productDetail) {
    contition = {detail: new RegExp(`^.*${productDetail}.*$`)}
  }else if (productPrice) {
    contition = {price: new RegExp(`^.*${productPrice}.*$`)}
  }else if (productStatus) {
    contition = {status: new RegExp(`^.*${productStatus}.*$`)}
  }
